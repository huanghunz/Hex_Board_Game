...
ostream& operator<< (ostream &out, Graph &graph)
{
	...
	for (int v = 0; v < graph.GRAPH_SZ; v++){
		graph.node = graph.adj_list[v].begin();
		out << "\\ ";
		if (graph.node->player == -1){
			out << setfill('0') << setw(3) << v << "";
		}
		else{
			out << "  " << (char)(graph.node->player);
		}
...
int Graph::monteCalo()// monticalo 
{
	vector<vector<int>> possibleMoves;
	

	
	possibleMoves.resize(NUM_THREADS);
	for (int j = 0; j < NUM_THREADS; j++){
		possibleMoves[j].resize(GRAPH_SZ);
	}
	time_t rawtime,end;
	time(&rawtime);
...
#pragma omp parallel num_threads(NUM_THREADS)
			{
				...
				for (int loop = 0; loop < LOOP / NUM_THREADS; loop++){ // each thread does the same thing
																	   // so divided the LOOP time by num threads
					list<Vertex>::iterator temp;
					Graph copy(*this);					//  copy it everytime

					temp = copy.adj_list[i].begin();	//make a play
					temp->player = PLAYER_2;
					copy.fillGraph();					//play to the end of the game
					if (copy.findAIMoves(i)){			// check to see who won the game
						possibleMoves[id][i]++;			//number of times you won for the ith position
					}
				}// end for loop
			}// end thread 
		}// end if
	...
	for (int k = 0; k < GRAPH_SZ; k++){
		for (int n = 0; n < NUM_THREADS; n++){
			probability[k] = possibleMoves[n][k]; // adds up all the possible movement from threading
		}
	}
